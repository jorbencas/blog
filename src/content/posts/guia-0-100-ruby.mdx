...
- **VSCode**: extensión Ruby de Shopify (mejor LSP), Ruby Solargraph para autocompletado. Instala también Endwise (auto-end), Sorbet (type checking).
- **RubyMine**: IDE dedicado de JetBrains, depurador visual excelente, refactors automáticos. De pago pero con trial.
- **Neovim**: con nvim-lspconfig + solargraph, ruby-lsp, o el LSP de Shopify. `vim-endwise` para `end` automático.
...
**BigDecimal** (requiere `require "bigdecimal"`):
...
Las strings en Ruby son **mutables por defecto**. Para inmutabilidad usa `freeze`:
...
Los símbolos son **inmutables e internados** (únicos en memoria):
...
Los arrays son **dinámicos y pueden contener tipos mixtos**:
...
`nil` es el **único objeto de NilClass**. Representa la ausencia de valor:
...
Ruby usa **GC generacional (mark-and-sweep)** desde Ruby 2.1:
...
**Todo es referencia a objeto:** Ruby no tiene tipos por valor. Cada variable guarda una referencia:
...
Fuente: [Ruby-Doc — Core](https://ruby-doc.org/core/), [Ruby-Doc — GC](https://ruby-doc.org/core/GC.html), [ObjectSpace](https://ruby-doc.org/core/ObjectSpace.html)
---
