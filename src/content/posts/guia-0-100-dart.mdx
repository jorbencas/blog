...
- **VSCode**: extensión "Dart" de Dart Code (dart-code.dart). Incluye análisis en vivo, autocompletado, refactors, debugger. Extensión "Flutter" adicional si usas Flutter.
- **Android Studio / IntelliJ**: plugin oficial de Dart/Flutter. Excelente depurador visual y hot reload.
- **Neovim/LSP**: `dartls` (Dart language server) vía `dart language-server --protocol=lsp`. Configurable con `mason.nvim`.
...
void procesar(Object obj) {
  if (obj is String) print(obj.length); // Promovido a String

  String? texto;
  if (texto != null) print(texto.length); // Promovido a String
}
```

### dynamic vs Object vs void vs Never

```dart
dynamic d = 42;       // Sin verificación de tipos en compile-time
d = 'hola';           // OK
d.metodoRaro();       // Solo falla en runtime

Object o = 42;        // Tipo base conocido, requiere cast
if (o is String) print(o.length); // Promovido

void v;               // Función que no devuelve valor útil

Never nunca() => throw UnimplementedError(); // Nunca retorna
```
...
### String (UTF-16, Runes, interpolación)
...
- **Dart VM generational GC**: new space (objetos jóvenes, se recolectan rápido) y old space (objetos que sobreviven varias colecciones). El GC es generacional: la mayoría de objetos muertos se recolectan en new space con un barrido rápido.
- **Isolates**: cada Isolate tiene su propio heap aislado. No comparten memoria, se comunican por paso de mensajes (SendPort/ReceivePort). Esto elimina carreras de datos y locks.
- **AOT (Ahead-Of-Time)**: compila a código nativo ARM/x64. Arranque rápido, rendimiento predecible. Para producción.
- **JIT (Just-In-Time)**: compila durante el desarrollo. Permite hot reload. Máxima velocidad de iteración.

Fuente: [dart.dev — Built-in Types](https://dart.dev/language/built-in-types), [dart.dev — Records](https://dart.dev/language/records), [dart.dev — Enums](https://dart.dev/language/enums), [dart.dev — Sealed classes](https://dart.dev/language/class-modifiers#sealed), [dart.dev — Extension types](https://dart.dev/language/extension-types), [Dart VM — mrale.ph](https://mrale.ph/dartvm/)
