...
|---------|--------|--------------|
| [**Machine Learning**](https://www.ibm.com/topics/machine-learning) | Programas que "aprenden" patrones a partir de datos en vez de seguir reglas escritas a mano | Detección de spam, recomendadores |
| [**Deep Learning**](https://www.ibm.com/topics/deep-learning) | Subconjunto del ML que usa redes neuronales con muchas capas | Reconocimiento de imágenes, voz, traducción |
...
- Durante el **entrenamiento**, la red produce un resultado, se compara con el esperado mediante una **función de pérdida** y se ajustan los pesos en la dirección que reduce el error ([**backpropagation**](https://www.3blue1brown.com/neural-networks)).
...
- **Encoder-only** (ej. [**BERT**](https://huggingface.co/docs/transformers/model_doc/bert), que aprendía con *masked language modeling*: ocultar palabras al azar y predecir cuáles son): lee toda la secuencia de golpe (bidireccional) y produce embeddings ricos. Ideal para clasificar, extraer o crear vectores de búsqueda (¡los de la sección 5!).
- **Decoder-only** (todo el **GPT**: autoregresivo, A *B* C → predice D). Genera texto. Es la familia dominante de LLMs de chat.
- **Encoder-decoder** (ej. [**T5**](https://huggingface.co/docs/transformers/model_doc/t5), [**BART**](https://huggingface.co/docs/transformers/model_doc/bart)): añade un paso de compresión intermedio; excelente para resumir o traducir.
...
- **[Perplexity](https://huggingface.co/docs/transformers/perplexity)**: lo bien que un modelo predice un texto. Mide "fluidez", no verdad. Cuanto más bajo, mejor predice el modelo ese texto concreto.
- **Benchmarks**: [MMLU](https://huggingface.co/datasets/cais/mmlu), [HumanEval](https://huggingface.co/datasets/openai_humaneval), [GSM8K](https://huggingface.co/datasets/openai/gsm8k)… miden conocimiento o razonamiento en condiciones controladas. Ojo: se saturan rápido y pueden estar "contaminados" (ya vistos en el entrenamiento).
