...
description: "Entrenador de mecanografía para desarrolladores: 100 niveles, 15 minijuegos, 4 idiomas, multijugador en tiempo real, foro comunitario y más de 50 componentes. React + TypeScript + Express + PostgreSQL + Docker."
pubDate: "2026/05/20"
updatedDate: "2026/07/28"
tags: ['web', 'react', 'desarrollo', 'tech', 'frontend', 'Javascript', 'docker', 'postgresql']
...
*   **Capa de Presentación (Frontend):** Construida con [**React 18**](https://react.dev/) y fuertemente tipada con [**TypeScript**](https://www.typescriptlang.org/), empleando [**Tailwind CSS**](https://tailwindcss.com/) para un sistema de diseño modular.
*   **Manejo de Estado Global:** Se delegó el árbol del estado a [**Zustand**](https://github.com/pmndrs/zustand) para minimizar renderizados innecesarios, asegurando que las métricas de rendimiento del usuario operen de manera aislada sin detonar cascadas de re-renders.
*   **Servicio de Backend y WebSockets:** La API Core está montada sobre [**Express 5**](https://expressjs.com/). Se integra un canal bidireccional de baja latencia con [**Socket.io**](https://socket.io/) responsable de persistir analíticas en vivo y manejar iteraciones de sincronización entre clientes.
*   **Capa de Persistencia y Gobernanza:** Los esquemas relacionales se ejecutan sobre una instancia de [**PostgreSQL 16**](https://www.postgresql.org/) contenedorizada con Docker, con volumen persistente para no perder datos entre reinicios. La autenticación es regulada mediante [**JWT**](https://jwt.io/) con sesiones persistidas en base de datos, autenticación OAuth 2.0 ([**Passport.js**](https://www.passportjs.org/)) y cifrado con [**Bcrypt**](https://en.wikipedia.org/wiki/Bcrypt). El canal WebSocket ([**Socket.IO**](https://socket.io/)) gestiona en tiempo real el foro comunitario (posts, comentarios, likes), el estado online de amigos y la sincronización entre clientes en salas multijugador.
...
*   **Testing E2E Automatizado:** Los scripts de regresión integrales iteran gracias a los flujos controlados implementados con [**Playwright**](https://playwright.dev/).
*   **Componentes a Prueba de Fallos:** Se provee un set de unit testings estrictos utilizando de forma híbrida el runtime de [**Jest**](https://jestjs.io/) junto a React Testing Library.
...
| Entrar al contenedor del frontend | `docker-compose exec frontend sh` |
| Ver contenedores en ejecución | `docker-compose ps` |
| Ver todos los contenedores | `docker-compose ps -a` |
| Limpiar imágenes sin usar | `docker image prune -f` |
...
Para desplegar el proyecto sin costes, se pueden usar: [**Hugging Face**](https://huggingface.co/) como hosting (Spaces Docker gratis), [**Neon**](https://neon.tech/) como base de datos PostgreSQL serverless (capa gratuita generosa) y [**GitHub**](https://github.com/) para el repositorio y CI/CD con Actions.
...
| `/api/auth/*` | POST/GET | No | Registro, login, Google OAuth |
| `/api/users` | GET | Admin | Listar todos los usuarios |
| `/api/users/:id` | PATCH | Admin | Actualizar usuario |
| `/api/users/:id` | DELETE | Admin | Eliminar usuario |
| `/api/settings` | GET/PATCH | Admin | Configuración global |
| `/api/stats/:userId` | GET | Auth | Estadísticas de usuario |
| `/api/stats/:userId/predictions` | GET/POST/PATCH | Auth | Predicciones de progreso |
| `/api/user-xp/*` | GET/POST | Auth/Admin | Sistema de XP |
| `/api/social/*` | GET/POST/DELETE | Auth | Foro, amigos, comentarios |
| `/api/challenges/*` | GET/POST | Auth | Desafíos diarios |
| `/api/multiplayer/*` | GET/POST | Auth | Salas multijugador |
| `/api/admin/*` | GET/DELETE | Admin | Estadísticas admin |
| `/api/preferences` | GET/PATCH | Auth | Preferencias (sync localStorage↔DB) |
| `/api/classes/*` | GET/POST/DELETE | Auth | Sistema de clases |
| `/api/activity` | GET | Auth | Logs de actividad |
| `/api/notifications` | GET/PUT/DELETE | Auth | Notificaciones |
| `/api/custom-levels` | GET/POST/PUT/DELETE | Auth | Niveles personalizados |
...
| Hooks custom | 15+ |
| Stores Zustand | 3 |
| Archivos backend | 20+ rutas |
| Tablas PostgreSQL | 26 |
| Idiomas soportados | 4 (es/en/ca/va) |
| Minijuegos | 15 |
| Logros desbloqueables | 10 |
| Niveles del currículo | 100 |
| Contenedores Docker | 3 (frontend/backend/db) |
