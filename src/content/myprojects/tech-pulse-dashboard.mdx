...
- **Traducción inteligente:** solo se traducen los elementos nuevos mediante el flag `traducido`, omitiendo los ya procesados
- **Resúmenes persistidos:** se almacenan en `noticias_historico.json` para evitar re-summarización en cada ejecución
...
- **Traducción inteligente:** reutiliza el título traducido si existe, traduce únicamente los nuevos
- **Resúmenes persistidos:** el campo `n['resumen']` se guarda para no re-summarizar
...
> **Actualizado en septiembre de 2026:** 515 fuentes con 89 GitHub Topics y 4 fuentes duales. Markdown Tools (PDF, Slides, Table, Snippets, Cheatsheet) integradas en el dashboard. Tips de IT con 283 conceptos. Herramientas IA con 150 categorías y auto-detección. AI Agent Skills: 68 skills (GitHub + mattpocock/skills ★258k). Blog resources migrados (2108). 134 tests con pytest.

## 4. Desglose del Feature Set

- **515 fuentes monitorizadas:** 158 canales de YouTube, 151 feeds RSS, 43 sitios de web scraping, 64 sitios de HTML scraping, 4 fuentes duales, 89 GitHub Topics, 8 repositorios y 1 Product Hunt
- **Clasificación inteligente:** 6 categorías temáticas (IA, Programación, DevOps, Ciberseguridad, Hardware, Negocios) con badges Tech/RSS
- **Dashboard SSR:** Python genera el HTML completo del servidor; JavaScript gestiona únicamente la interactividad (~135 líneas)
- **Configuración externalizada:** prompts de IA, plantillas de Telegram y email, canales YouTube, opciones de filtro — todo centralizado en `constants_sources.py` y `constants_templates.py`
- **Constantes con nombre descriptivo:** todas las claves de acceso a diccionarios del esquema son constantes, lo que permite modificar un campo editando una única línea
- **Sección de Referencia Tech:** panel colapsable con Skills, LLMs, Lenguajes, Frameworks y Librerías
- **Newsletter diaria:** agrupada por fuente, con resúmenes individuales por noticia generados por Gemini y sección de vídeos
- **Telegram solo noticias:** sin vídeos, con audio TTS a las 21:00 UTC que incluye todas las noticias del día
- **Traducción inteligente:** el flag `traducido` se persiste para evitar re-traducción de elementos ya procesados
- **Resúmenes persistidos:** el campo `n['resumen']` se almacena en el JSON para no re-summarizar
- **Fuentes duales:** extracción simultánea de YouTube y web scraping (MoureDev, Midudev, Carlos Azaustre, Xataka)
- **Ranking GitHub Stars:** top 20 repositorios ordenados por estrellas, extraídos de 89 GitHub Topics y Collections
- **Deduplicación:** por URL normalizada y título similar durante el scraping, la generación del dashboard y la deduplicación entre archivos de resources
- **Estructura modular reorganizada:** scripts en subdirectorios (`scrapers/`, `publishers/`, `tools/`, `utils/`, `solutions/`)
- **Constantes reorganizadas:** `constants_sources.py` (fuentes), `constants_templates.py` (HTML, email, prompts), `constants_mdtools.py` (plantillas de Markdown Tools)
- **134 tests con pytest:** cobertura completa de utilidades, caché, constantes (incluyendo plantillas de email), imágenes, recursos y soluciones
- **CI/CD completo:** 15 workflows de GitHub Actions con badges en vivo en el README
- **Tips de IT:** 283 conceptos con preguntas de entrevista, generados con Gemini y verificados con Wikipedia y fallback local
- **Herramientas IA:** 150 categorías con auto-detección de herramientas (HuggingFace + GitHub + Product Hunt)
- **AI Agent Skills:** 68 skills de GitHub y mattpocock/skills (★258k) para mejorar las respuestas del bot
...
## 7. Arquitectura CSS del Frontend (Septiembre 2026)
