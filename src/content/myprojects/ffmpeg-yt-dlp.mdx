...
**[CRF](https://trac.ffmpeg.org/wiki/Encode/H264)** (Constant Rate Factor): valor de 0-51 donde menor = mejor calidad. 18 es prácticamente indistinguible del original.
...
- [**NVENC**](https://developer.nvidia.com/video-codec-sdk) (NVIDIA): Si `nvidia-smi` está disponible → usa `h264_nvenc` / `hevc_nvenc`
- [**VAAPI**](https://github.com/intel/libva) (Intel/AMD): Si `vainfo` está disponible → usa `h264_vaapi`
...
¿Cómo instalar y configurar midu.sh, el monitor y el preview watcher? Consulta la [guía de instalación completa](/posts/instalacion-ffmpeg) — con Docker, sin Docker, monitor de compresión y configuración de notificaciones.
