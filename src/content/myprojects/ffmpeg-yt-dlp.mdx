...
El proyecto define el servicio Docker **`monitor`** (`ffmpeg_monitor`), pensado para el pipeline de compresión automática:
...
Vigila `/home/jorge/dev/devjobs/test_videos/test` (donde TwitchRecorder deja los `*_completed.mp4`), comprime a 720p y guarda en `/home/jorge/dev/devjobs/Videos/comprimidos`. Es la pieza central del pipeline **"Grabar → Comprimir → Subir a Telegram"**.
